...
SELECT
  continterno,
  CAST(NULL AS VARCHAR2(50)) idagn,
  CAST(NULL AS NUMBER) corre1porcomiventa,
  cosecha,
  areageo,
  tipoentrega,
  CAST(NULL AS NUMBER) uvaluepauta,
  CAST(NULL AS NUMBER) ol_precio,
  CAST(NULL AS NUMBER) ol_pizarra,
  CAST(NULL AS NUMBER) tariflete,
  date_c(fecent),
  date_c(fecvtoent),
  compcuit,
  centro,
...
SELECT
  ...
  date_c(fecvtoent),
  date_c(fecfijahasta),
  CAST(NULL AS VARCHAR2(10)) horafijar,
  CAST(NULL AS NUMBER)  impadic,
  CAST(NULL AS NUMBER)  monfijada,
  contrato,
  areageo,
  tipoentrega,
  CAST(NULL AS NUMBER)  campo4,
  operacion,
  grano,
  ttmaxpact,
  zona
